...
class Map extends Component{
    ...
    componentDidMount() {
        ...
        googleMapScript.addEventListener('load',()=> {
            ...
            this.dirService=new window.google.maps.DirectionsService();
            this.dirRenderer=new window.google.maps.DirectionsRenderer({suppressMarkers:true});
            this.dirRenderer.setMap(this.googleMap)
            ...
            this.googleMap.addListener('click', (mapsMouseEvent)=>{
                ...
                this.geocoder.geocode({'location':mapsMouseEvent.latLng},(results,status)=>{
                    if (status === 'OK'){
                        ...
                        infowindow.open(this.googleMap, this.userMarker);
                        this.zoomMap();
    ...
    zoomMap=()=> {
        ...
        this.googleMap.setCenter(this.userMarker.position);
    }
    generateRandomCoordinate=()=> {
        var location = new window.google.maps.LatLng(
            this.userMarker.getPosition().lat() + (Math.random() / 100 * ((Math.round(Math.random()) % 2) ? 1 : -1)),
            this.userMarker.getPosition().lng() + (Math.random() / 100 * ((Math.round(Math.random()) % 2) ? 1 : -1))
        );
        this.generatedMarker = new window.google.maps.Marker({
            position: location,
            map: this.googleMap
        });
    }
    highlightTDMap=()=> {
        // highlight a street
        var request = {
            origin: this.userMarker.getPosition().lat() + "," + this.userMarker.getPosition().lng(),
            destination: this.generatedMarker.getPosition().lat() + "," + this.generatedMarker.getPosition().lng(),
            travelMode: window.google.maps.TravelMode.DRIVING
        };
        this.userMarker.setMap(null);
        this.generatedMarker.setMap(null);
        this.dirService.route(request, (result, status)=> {
            if (status == window.google.maps.DirectionsStatus.OK) {
                this.TDMapRoute = result.routes[0];
                this.dirRenderer.setDirections(result);
                new window.google.maps.Marker({
                    position: this.TDMapRoute.overview_path[0],
                    map: this.googleMap,
                    icon: "http://maps.google.com/mapfiles/kml/pal2/icon13.png"
                });
                new window.google.maps.Marker({
                    position: this.TDMapRoute.overview_path[this.TDMapRoute.overview_path.length - 1],
                    map: this.googleMap,
